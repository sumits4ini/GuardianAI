...
import { MapPin, Clock, AlertTriangle, TrendingUp, ChevronRight, Layers } from "lucide-react";
...
export function RecentActivity({ reports }: RecentActivityProps) {
  ...
  return (
    <div className="p-4 rounded-2xl glass-panel border border-slate-800 space-y-3">
        ...
        {reports.length > 0 ? (
          reports.slice(0, 4).map((r) => (
            <div key={r.id} className="p-2.5 rounded-xl bg-slate-900/80 border border-slate-800 text-xs space-y-1">
              <div className="flex items-center justify-between">
                <span className="font-bold text-white capitalize">{r.category.replace('_', ' ')}</span>
      ...
      <Link
        href="/map"
        className="w-full py-2 rounded-xl bg-slate-900 hover:bg-slate-800 border border-slate-700 text-xs font-bold text-slate-200 flex items-center justify-center gap-1.5 transition-colors block text-center"
      >
