...
import { X, ShieldCheck, Clock, MapPin, Sparkles, Navigation, AlertTriangle, ArrowRight, Check } from "lucide-react";
...
export function RouteComparisonModal({
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-950/80 backdrop-blur-md animate-in fade-in">
      <div className="relative w-full max-w-2xl bg-slate-900 border border-slate-800 rounded-3xl p-6 shadow-2xl space-y-5 overflow-hidden">
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-xs text-slate-400">
                {originName} <ArrowRight className="inline w-3 h-3 text-slate-500" /> {destinationName}
              </p>
            </div>
          </div>
        ...
        {loading ? (
          <div className="py-12 text-center space-y-2">
            ...
            <p className="text-xs text-slate-400">Evaluating lighting, foot traffic, and incident hazards...</p>
          </div>
        ) : data ? (
          <div className="space-y-4">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3.5">
              {data.routes.map((route: any) => {
                ...
                return (
                  ...
                  >
                    {isSafe && (
                      <span className="absolute -top-2.5 right-4 text-[9px] font-black uppercase tracking-wider px-2 py-0.5 rounded-full bg-emerald-500 text-slate-950 shadow-sm flex items-center gap-1">
                    ...
                    <div className="space-y-2.5">
                      <div className="flex items-start justify-between">
                        <h4 className="text-xs font-bold text-white pr-6">{route.name}</h4>
                      ...
                      <div className="flex items-center gap-3 text-xs">
                        ...
                        <div className="text-slate-400 text-[11px]">{route.distanceKm} km</div>
                        <span className="text-slate-600">•</span>
                      ...
                      <div className="pt-2 border-t border-slate-800/80 space-y-1.5 text-[11px]">
                        <div>
                          ...
                          <span className="text-slate-200">{route.lightingRating}</span>
                        </div>
            ...
            {/* AI Explanation Box */}
            {data.aiExplanation && (
              <div className="p-3.5 rounded-2xl bg-indigo-950/40 border border-indigo-500/30 space-y-1 text-xs">
